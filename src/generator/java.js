module.exports = {
  ...
  */
    ...
    if (property.hasOwnProperty("langType") && property.langType.hasOwnProperty("java")) {
      ...
      switch (property.type) {
        ...
        case "array":
          ...
          type = "java.util.List<" + property.items["$ref"].replace("#/primitives/", "") + ">";
          //type = "java.util.List<" + property.items["$ref"].replace("#/primitives/", this.namespace + ".") + ">";
          break;
        case "object":
          type = name;
      ...
      type = property["$ref"].replace("#/primitives/", "");
      //type = property["$ref"].replace("#/primitives/", this.namespace + ".");
    ...
    if (schema.hasOwnProperty("allOf")) {
      ...
      schema.allOf.forEach(function(item, index) {
        ...
        if (item.hasOwnProperty("$ref")) {
          ...
          refs += item["$ref"].replace("#/primitives/", "");
        } else {
  ...
  recurseInterfaces: function(api, item, properties, privates, name) {
    ...
    propName = item["$ref"].replace("#/primitives/", "");
  ...
  createApiPOJO: function(api, name, schema) {
    ...
    if (schema.hasOwnProperty("allOf")) {
      ...
      schema.allOf.forEach(function(item, index) {
        ...
        if (item.hasOwnProperty("$ref")) {
          if (index === 0) {
            refs = " extends " + that.interfaceToClassName(item["$ref"].replace("#/primitives/", ""));
          ...
          //   refs += ", ";
          // }
          //  var refName = item["$ref"].replace("#/primitives/", "");
          if (index === 0) {
  ...
  writeFile: function(name, content, extension, altPath) {
    var dir = this.distPath,
      that = this;
      if(!altPath){
    if (!this.fs.existsSync(dir)) {
      this.fs.mkdirSync(dir);
    }
    dir =  this.distPath+"/java";
    if (!this.fs.existsSync(dir)) {
      this.fs.mkdirSync(dir);
    }
    dir =  this.distPath+"/java/org";
    if (!this.fs.existsSync(dir)) {
      this.fs.mkdirSync(dir);
    }
    dir =  this.distPath+"/java/org/cmapi/";
    if (!this.fs.existsSync(dir)) {
      this.fs.mkdirSync(dir);
    }
    dir =  this.distPath+"/java/org/cmapi/primitives";
    if (!this.fs.existsSync(dir)) {
      this.fs.mkdirSync(dir);
    }
  } else {
  ...
  generate: function(schema, namespace, debug, distPath) {
    var primitives = schema.primitives,
      key,
      composite1="",
      composite2="";
    if(distPath){
    ...
    for (key in primitives) {
      composite1 += "\n\n"+this.createApiInterface(primitives, key, primitives[key]);
      composite2 += "\n\n"+this.createApiPOJO(primitives, key, primitives[key]);
    }
    this.writeFile("cmapi2.java", composite1+"\n\n\n"+composite2, ".txt", "dist");
  }
};
