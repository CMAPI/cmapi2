    ...
    */
        ...
        if (property.hasOwnProperty("langType") && property.langType.hasOwnProperty("js")) {
            ...
            switch (property.type) {
                ...
                case "string":
                    ...
                    break;
                case "array":
                    type = "cmapi." + property.items["$ref"].replace("#/primitives/", "") + "[]";
                    break;
                case "object":
                    type = "cmapi."+name;
                    break;
            ...
            type = "cmapi."+property["$ref"].replace("#/primitives/", "");
        ...
        if (schema.hasOwnProperty("allOf")) {
            ...
            schema.allOf.forEach(function(item, index) {
                ...
                if (item.hasOwnProperty("$ref")) {
                    if (index === 0) {
                        refs = that.interfaceToClassName(item["$ref"].replace("#/primitives/", ""));
    ...
    generate: function(schema, namespace, debug, distPath) {
        ...
        for (key in primitives) {
            composite += this.createApiPOJO(primitives, key, primitives[key]);
            newEnum = this.generateEnums(primitives, key, primitives[key]);
            if (newEnum.length > 0) {
        ...
        this.writeFile("org.cmapi.primitives", composite + this.getInherit() + uuidFunc, ".js", "src/docs/js");
        this.writeFile("cmapi2.schema", "var cmapi2 = "+JSON.stringify(schema,undefined,2), ".js", "src/docs/js");
